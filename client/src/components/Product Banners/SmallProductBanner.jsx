    ...
    <div className=' relative h-[580px] cursor-pointer'>
        <div className={`${props.banner?.blackHeader ? 'apple_black' : 'text-white'}`}> 
            ...
            <div className={`absolute ${props.banner.bottom ? 'bottom-12' : 'top-12'} left-0 right-0 w-fit mx-auto flex flex-col items-center`}>
                {props.banner?.header
                ? 
                <h1 className='text-4xl font-semibold mb-2'>{props.banner?.header}</h1>
                : 
                <img className='mb-2' src={props.banner?.headerArt} alt="" />
                }
                <h2 className='text-xl'>{props.banner?.subheader}
                    {props.banner.sup && <sup>{props.banner.sup}</sup>}
                </h2>

                {props.banner.customButton
                ?
                <button className={`rounded-full mt-4 text-sm ${props.banner?.whiteButton ? 'bg-[#f3f6f6] hover:bg-white text-black' : 'bg-[#0071e3] hover:bg-[#0077ED] text-white' } py-2 px-5 mr-4`}>{props.banner.customButton}</button>
                :
